...
export async function createSession(userId: string, name: string, isAdmin: boolean, isSuperAdmin: boolean) {
  const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) // 7 days
  const session = await encrypt({ userId, name, isAdmin, isSuperAdmin })
